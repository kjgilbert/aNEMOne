...
#'
#'  @param temp.gen If temporal capacities are desired, this is the generation at which the provided capacities will be set.
#'
#'  @return
#'
...
patch.cap <- function(K1, num.K1, K2=NULL, num.K2=NULL, temp.gen=NULL){
	...
	if(!is.null(K2)){
		...
		if(!is.null(temp.gen)) cap.array <- paste(paste(c("@g", temp.gen), collapse=""), "{{", capacities, "}}")
	}else{
		cap.array <- paste("{{", first.values, "}}")
		if(!is.null(temp.gen))  cap.array <- paste(paste(c("@g", temp.gen), collapse=""), "{{", first.values, "}}")
	}
	return(cap.array)
}
