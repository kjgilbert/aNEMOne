...
plot1D.results <- function(input.fit.file, input.quanti.file=NULL, input.del.file=NULL, patches.x, patches.y, slope.opt=NULL, opt.rate.change=NULL, generation, del.loci, xlimits=NULL, plot.legend=TRUE){
  ...
  if(!is.null(input.quanti.file) & !is.null(slope.opt)){
    ...
    plot(1:patches.x, env, xlim=xlimits, ylim=c(min(avg.quanti$P1)-10, max(avg.quanti$P1)+10), type="l", lwd=1.5, xlab="Landscape x position", ylab="Quanti trait & env. optimum", main=paste(c("Generation ", generation), collapse=""))
